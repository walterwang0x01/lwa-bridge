...
## 新发现的问题（2026-07-22，用户反馈）：docked 模式完全不支持历史滚动查看

**现象**：用户反馈"回答后的问题记录不支持上下滚动查看"。

**已确认（读代码，非猜测）**：`layoutContentViewport()`
（`src/ingress/cli/shellScreen.ts` 70 行附近）用 `opts.lines.slice(-height)`
——永远只取 `transcript` 最后 `height`（终端可视高度）行，**没有任何
`scrollOffset` 参数，没有实现应用层滚动**。同时 docked 模式用
`\x1b[?1049h`（alt-screen buffer）接管全屏渲染，这会让终端原生的
scrollback（鼠标滚轮/Shift+PageUp）机制失效——因为程序在用绝对定位
不断重绘同一个屏幕区域，不是真的往下追加输出。两者叠加的结果：
**一旦对话历史超出当前终端窗口高度，更早的内容变得完全不可访问**，
没有任何操作（鼠标滚轮、快捷键）能看到它们，除非退出 docked 模式。

这不是 bug，是这个功能从未被实现过。docked 模式设计文档注释里写的
...
| 任务 | Spec | 修改文件 | 状态 |
|------|------|----------|------|
| `update-progress-md` | `.conduit-plan/specs/update-progress-md.md` | `PROGRESS.md` | ⏳ 待执行 |
| `conduit-proactive-suggestion` | `.conduit-plan/specs/conduit-proactive-suggestion.md` | `src/core/dispatcher.ts` + 测试 | ⏳ 待执行 |
| `gemini-cli-verification` | `.conduit-plan/specs/gemini-cli-verification.md` | `docs/spikes/gemini-cli-verify.md` + `scripts/smoke-gemini-cli.mts` | ⏳ 待执行 |

### 备注

- `conduit-proactive-suggestion`：在 `handle()` step 7 之前插入 `maybeEmitConduitSuggestion`，
  仅 CLI 文本通道、非命令消息、含 ≥3 独立任务信号时触发，24h 防重复。
- `gemini-cli-verification`：补验收文档 + 自动 smoke 脚本（gemini 未安装时 skip）。
- `update-progress-md`：把 `fca08b9` 里的"按键才吐字"修复节标记为「已完成」，
  清除误导性的"下一步（还没做）"表述。

跑方式：`/conduit run`（或 `cd .conduit-plan && lwa-conduit run`）。
