...
`kiro-cli` 和 `lark-cli` 的）。conduit 要被串联，唯一要求是 `kiro-conduit`
命令在 PATH 上。

`kiro-conduit/pyproject.toml` 已配好入口（已验证）：

```toml
[project.scripts]
kiro-conduit = "kiro_conduit.cli:main"
```
...
| PyPI | `pip install kiro-conduit` | 公开分发给别人 |
...
1. spawn `kiro-conduit run <spec>` 子进程（同 kiro-cli 的 spawn 模式）
2. 解析其输出/事件
